...
async function main() {
  ...
  try {
    ...
    const deployedErc20Address = await endpointContract.getAddressByResourceId(
      resourceId
    );
    logInfo(
      `  Endereço retornado pelo Endpoint para o Resource ID '${resourceId}': ${deployedErc20Address}`
    );
    ...
    let tokenSymbol = "TKN";
    try {
      tokenSymbol = await playgroundErc20Contract.symbol();
